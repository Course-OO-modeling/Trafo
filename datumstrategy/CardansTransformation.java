 ...
 * Needed parameter for the Transformation: 3 translation-, 1 scale- and 3 rotation -parameters.
 ...
 * The Cardan's Rotation is done with trigonometric funtion. 
 * After the Transformation, the new coordinates are converted into ellipsoidal coordinates by an iterative process.
 * 
 * source of cardan's rotation: http://www.itm.uni-stuttgart.de/courses/madyn/Merkblaetter/M08.pdf (in Germany)
 * Concept explanation : http://n.ethz.ch/~roclaudi/download/4.Semster/glossarium%20h%F7ge.pdf
 * @author Yucheng Luo
...
public class CardansTransformation extends TransformationStrategy {
	 ...
	 * 
	 * Translation : Cartesian coordinates are used.
	 * Rotation : rotate with principle of conventional Cardan's Rotation (Wx,Wy,Wz).
     * Algorithm explanation : Rotate first about X-axis ,after first step rotate about current Y-axis ,at end rotate about current Z-axis.
     * 
     * @author Yucheng Luo
	 */
	@Override
    public void transform(GeographicCoordinateInterface geo) {
    	...
    	e4 =  Math.cos(control.getWx())*Math.sin(control.getWz())+Math.sin(control.getWx())*Math.sin(control.getWy())*Math.cos(control.getWz());
    	e5 =  Math.cos(control.getWx())*Math.cos(control.getWz())-Math.sin(control.getWx())*Math.sin(control.getWy())*Math.sin(control.getWz()) ;
    	e6 = -Math.sin(control.getWx())*Math.cos(control.getWy());
    	e7 =  Math.sin(control.getWx())*Math.sin(control.getWz())-Math.cos(control.getWx())*Math.sin(control.getWy())*Math.cos(control.getWz()) ;
    	e8 =  Math.sin(control.getWx())*Math.cos(control.getWz())+Math.cos(control.getWx())*Math.sin(control.getWy())*Math.sin(control.getWz());
    	e9 =  Math.cos(control.getWx())*Math.cos(control.getWy());
    	
    	XYZDestination.setX(control.getDx() + control.getMassstab() * (XYZSource.getX()*e1 + XYZSource.getY()*e2 + XYZSource.getZ()*e3));
    	XYZDestination.setY(control.getDy() + control.getMassstab() * (XYZSource.getX()*e4 + XYZSource.getY()*e5 + XYZSource.getZ()*e6));
    	XYZDestination.setZ(control.getDz() + control.getMassstab() * (XYZSource.getX()*e7 + XYZSource.getY()*e8 + XYZSource.getZ()*e9));
