...
public class GaussKruegerTest {
	...
	/**
	 * @brief tests method getAsGeographic from class GaussKrueger, overwritten from
	 *        abstract class coordinate expected values and input values are taken
	 *        from book "Rechenformeln und Rechenbeispiele zur Landesvermessung -
	 *        Teil 2" from Prof.Dr.-Ing. Albert Schoedlbauer, Robert Wichmann Verlag
	 *        Karlsruhe, page 88
	 */
	@Test
	public void getAsGeographic() {
		...
		Assert.assertEquals(String.format("Latitude %s of result coordinate does't match expected value %s.",
				resultLatitude, expectedLatitude), expectedLatitude, resultLatitude, EPSILON_IN_DEGREE);
		Assert.assertEquals(String.format("Longitude %s of result coordinate does't match expected value %s.",
				resultLongitude, expectedLongitude), expectedLongitude, resultLongitude, EPSILON_IN_DEGREE);
		Assert.assertEquals(String.format("Height %s of result coordinate does't match expected value %s.",
				resultHeight, expectedHeight), expectedHeight, resultHeight, EPSILON_IN_DEGREE);
	}
