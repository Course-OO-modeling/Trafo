 ...
 *  
 *  @brief concrete implementation of abstract class Coordinate, represents a geographic coordinate (As interface for strategy)<br/>
 *	@author unknown as of 2016-11
 *	@remark renamed to GeographicCoordinateInterface. Class is now separated into GeographicCoordinate (only input coordinate) and GeographicCoordinateInterface (Interface for strategy) on 2018-01-12 by Johanna Stoetzer
...
public final class GeographicCoordinateInterface extends Coordinate {
    ...
    ******************************************************************************************************************/
    ...
    /**
     * @brief allows the particular GeographicCoordinate instance to be converted to GeographicCoordinate. simply returns this reference
    ...
    /**
     * @brief allows the the particular GeographicCoordinate instance to be converted from GeographicCoordinate. simply uses copy method
     ...
     * @param ell - an object of type EllipsoidParms that contains details about the ellipsoid the transformation refers to
     * @param trafoparameter - an object of type MolodenskyParm that contains the parameters of the Molodenskij transformation
     * @return geo_p2 - returns a transformed geographic coordinate
     */
    public GeographicCoordinateInterface molodenskytrafo(EllipsoidParms ell, MolodenskyParm
            trafoparameter) {
        ...
        /* * * * This standard formula is used in this program, because all parameters are available from NIMA * * * */
        ...
        dl = (-trafoparameter.getTransx() * Math.sin(longitude)
                + trafoparameter.getTransy() * Math.cos(longitude))
     ...
     * @param ell - an object of type EllipsoidParms that contains details about the ellipsoid the pulse width calculation refers to
     ...
     * @param ell - an object of type EllipsoidParms that contains details about the ellipsoid
    ...
    public void GeographicLongitudeLatitude(double xg, double yg, EllipsoidParms ell, double scale, double l0) {
        ...
        latitude = bf - .5 * latparm.getTbr() * ygrad2 * vbf2 * (1. - ygrad2 / 12. *
                (5. + 3. * tbf2 + latparm.getEtabr() - 9. * tbf2 * latparm.getEtabr() + ygrad2 / 30. *
                (61. + 90. * tbf2 + 45. * tbf2 * tbf2)));

        longitude = l0 + ygrad / Math.cos(bf) * (1. - ygrad2 / 6. * (vbf2
                + 2. * tbf2 - (5. + tbf2 * (28. + 24. * tbf2)) / 20. * ygrad2));

    } 

    /**
     * @brief works like a copy constructor, initializes particular instance with the values of the instance passed as parameter 
     * @param geo - the GeographicCoordinate to initialize the particular instance with
     */
